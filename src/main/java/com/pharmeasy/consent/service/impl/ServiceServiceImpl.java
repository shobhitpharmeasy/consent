    ...
    implements ServiceService
{
     ...
     */
    private final org.springframework.beans.factory.ObjectProvider<ServiceService>
        selfProvider;
    ...
    public ServiceDto createService(final ServiceDto serviceDto) {
        ...
        if (serviceDto.createdByEmail() == null) {
            missingFields.add("createdBy");
        }
        if (serviceDto.status() == null) {
            missingFields.add("status");
        }
        if (serviceDto.employeeAccess() == null) {
            missingFields.add("employeeAccess");
        }

        if (!missingFields.isEmpty()) {
            log.warn(
                "Missing required fields while creating service: {}",
                missingFields
                    );
        ...
        final Employee creator = employeeRepository
                                     .findById(serviceDto.createdByEmail())
                                     .orElseThrow(() -> {
                                         log.warn(
                                             "Creator not found: {}",
                                             serviceDto.createdByEmail()
                                                 );
                                         return new RuntimeException(serviceDto.createdByEmail());
                                     });
        ...
        for (final Map.Entry<String, AccessStatus> entry : serviceDto
                                                               .employeeAccess()
                                                               .entrySet()) {
            final Employee emp =
                employeeRepository.findById(entry.getKey()).orElseThrow(() -> {
                    log.warn(
                        "Employee not found for access mapping: {}",
                        entry.getKey()
                            );
    ...
    public ServiceDto updateService(final ServiceDto serviceDto) {
        log.info("Updating service: {}", serviceDto.name());
        if (serviceDto.name() == null) {
            throw new RuntimeException("Service name is required");
        }

        final Service existing =
            serviceRepository.findById(serviceDto.name()).orElseThrow(() -> {
                log.warn("Service not found for update: {}", serviceDto.name());
                return new RuntimeException(serviceDto.name());
            });

        if (serviceDto.description() != null) {
            existing.setDescription(serviceDto.description());
        }
        if (serviceDto.status() != null) {
            existing.setStatus(serviceDto.status());
        }
        if (serviceDto.createdByEmail() != null) {
            final Employee creator = employeeRepository
                                         .findById(serviceDto.createdByEmail())
    ...
    public String deleteService(final String serviceName) {
        ...
        final Service service =
            serviceRepository.findById(serviceName).orElseThrow(() -> {
    ...
    public String whoIsOwnerToService(final String serviceName) {
        ...
        return serviceRepository
                   ...
                   .map(s -> s
                                 .getCreatedBy()
                                 .getEmail())
                   .orElseThrow(() -> new RuntimeException(serviceName));
    }
    ...
    @Override
    @Transactional
    public Boolean requestServiceAccess(
        final String serviceName,
        final String email
                                       )
    {
        log.info(
            "Requesting service access: service={}, employee={}",
            serviceName,
            email
                );
        final Service service = serviceRepository
                                    .findById(serviceName)
        ...
        if (currentStatus == AccessStatus.REQUESTED ||
            currentStatus == AccessStatus.ACCEPTED)
        {
            log.info(
                "Service access already requested or granted for employee: {}",
                email
                    );
            return false;
        }
    ...
    {
        log.info(
            "Removing service access: service={}, employee={}",
            serviceName,
            email
                );
        final Service service = serviceRepository
                                         ...
                                         )
    {
        ...
        final Employee emp = employeeRepository
                                 .findById(email)
                                 .orElseThrow(() -> new RuntimeException(email));
        final boolean granted = AccessStatus.ACCEPTED.equals(service
                                                                 .getEmployeeAccess()
                                                                 .get(emp));
        log.debug(
            "Access granted for service={} and employee={}: {}",
            serviceName,
            email,
            granted
                 );
    ...
    {
        final boolean isOwner = self().whoIsOwnerToService(serviceName).equals(
            email)
            ;
    ...
    {
        log.info(
            "Transferring ownership of service={} to newOwner={}",
            serviceName,
            newOwnerEmail
                );
        final Service service = serviceRepository
                                    ...
                                    .orElseThrow(() -> new RuntimeException(
                                        serviceName));
        final Employee newOwner = employeeRepository
                                      .findById(newOwnerEmail)
                                      .orElseThrow(() -> new RuntimeException(
                                          newOwnerEmail));
        service.setCreatedBy(newOwner);
        serviceRepository.save(service);
        log.info("Service ownership transferred successfully");
        return "Service ownership transferred successfully to: " +
               newOwnerEmail;
    }
    ...
    {
        log.info(
            "Granting service access: service={}, employee={}",
            serviceName,
            email
                );
        final Service service = serviceRepository
    ...
    @Override
    @Transactional(readOnly = true)
    public List<String> whoHasRequestedMyService(
        final String serviceName,
        final String ownerEmail
                                                )
    {
        if (Boolean.FALSE.equals(self().isServiceOwnedByMe(
            serviceName,
            ownerEmail
                                                          )))
        {
            log.warn(
                "Unauthorized access to requesters list by: {}",
                ownerEmail
                    );
    ...
    @Override
    @Transactional(readOnly = true)
    public List<String> whoHasAccessToMyService(
        final String serviceName,
        final String ownerEmail
                                               )
    {
